...
export interface GoogleAdsMetricsError {
  ok: false;
  message: string;
}
...
export async function getGoogleAdsConfig(
  ...
  const row = await db.prepare(
    "SELECT config FROM Integration WHERE organizationId = ? AND slug = ? AND status = 'connected'"
  )
    .bind(organizationId, GOOGLE_ADS_SLUG)
    .first<{ config: string | null }>();
  if (!row?.config) return null;
  try {
    return JSON.parse(row.config) as GoogleAdsConfig;
  } catch {
    return null;
  }
}

export async function refreshAndSaveGoogleAdsToken(
  db: D1Database,
  organizationId: string,
  clientId: string,
  clientSecret: string,
  ...
  await db.prepare(
    "UPDATE Integration SET config = ?, lastSyncAt = ?, updatedAt = ? WHERE organizationId = ? AND slug = ?"
  )
    .bind(configStr, now, now, organizationId, GOOGLE_ADS_SLUG)
    .run();
...
async function listAccessibleCustomers(accessToken: string): Promise<string[]> {
  ...
  return names.map((r) => r.replace("customers/", ""));
}
...
async function searchStream(
  ...
  // searchStream pode retornar um objeto com results ou um array de batches
...
export async function fetchGoogleAdsMetrics(
...
): Promise<GoogleAdsMetricsResponse | GoogleAdsMetricsError> {
  const config = await getGoogleAdsConfig(db, organizationId);
  if (!config) {
  ...
  if (!env.GOOGLE_ADS_DEVELOPER_TOKEN) {
    return {
      ok: false,
      message: "Developer Token do Google Ads não configurado. Configure GOOGLE_ADS_DEVELOPER_TOKEN no Worker.",
    };
  }
  ...
  if (config.expiry_date && now >= config.expiry_date - margin) {
    if (!env.GOOGLE_CLIENT_ID || !env.GOOGLE_CLIENT_SECRET) {
      return { ok: false, message: "Credenciais Google OAuth não configuradas para renovar token." };
    }
    try {
      accessToken = await refreshAndSaveGoogleAdsToken(
        db,
        organizationId,
