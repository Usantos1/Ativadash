...
// .wrangler/tmp/bundle-jdwfoc/checked-fetch.js
...
// .wrangler/tmp/bundle-jdwfoc/strip-cf-connecting-ip-header.js
...
async function verifyPassword(password, stored) {
  try {
    ...
    const key = await deriveKey(password, salt);
    if (key.length !== keyStored.length)
      return false;
...
var ACCESS_TTL = "15m";
var REFRESH_TTL = "7d";
async function register(db, secrets, data) {
  ...
  await db.batch([
    ...
    ).bind(orgId, `${data.name} - Organiza\xE7\xE3o`, orgSlug, now, now),
    db.prepare(
...
async function login(db, secrets, data) {
  const user = await db.prepare("SELECT id, email, name, password FROM User WHERE email = ?").bind(data.email).first();
  if (!user)
  ...
  if (!membership)
    throw new Error("Usu\xE1rio sem organiza\xE7\xE3o vinculada");
  const { accessToken, refreshToken } = await createTokens(
    db,
    secrets,
    user.id,
...
__name(login, "login");
async function refreshAccessToken(db, secrets, refreshToken) {
  ...
  const membership = await db.prepare(
    "SELECT organizationId FROM Membership WHERE userId = ? LIMIT 1"
  ).bind(stored.userId).first();
  if (!membership)
    throw new Error("Usu\xE1rio sem organiza\xE7\xE3o");
...
async function createTokens(_db, secrets, userId, email, organizationId) {
  ...
  const accessToken = await new SignJWT({
    ...
    email,
    organizationId
  }).setProtectedHeader({ alg: "HS256" }).setExpirationTime(ACCESS_TTL).setIssuedAt().sign(secret);
  const refreshToken = await new SignJWT({ userId, type: "refresh" }).setProtectedHeader({ alg: "HS256" }).setExpirationTime(REFRESH_TTL).setIssuedAt().sign(refreshSecret);
  return { accessToken, refreshToken };
}
__name(createTokens, "createTokens");
async function saveRefreshToken(db, _refreshSecret, userId, token) {
  const { payload } = decodeJwt(token);
  const exp = payload?.exp ?? Math.floor(Date.now() / 1e3) + 7 * 24 * 3600;
  const expiresAt = new Date(exp * 1e3).toISOString();
  const id = crypto.randomUUID();
  await db.prepare(
    "INSERT INTO RefreshToken (id, token, userId, expiresAt, createdAt) VALUES (?, ?, ?, ?, ?)"
  ).bind(id, token, userId, expiresAt, (/* @__PURE__ */ new Date()).toISOString()).run();
}
...
async function saveState(db, state, organizationId) {
  const expiresAt = new Date(Date.now() + STATE_TTL_MS).toISOString();
  await db.prepare("INSERT INTO OAuthState (state, organizationId, expiresAt) VALUES (?, ?, ?)").bind(state, organizationId, expiresAt).run();
}
__name(saveState, "saveState");
async function consumeState(db, state) {
...
// src/integrations.ts
var GOOGLE_ADS_SLUG = "google-ads";
var SCOPES = ["https://www.googleapis.com/auth/adwords"];
async function getGoogleAdsAuthUrl(db, organizationId, apiBaseUrl, clientId) {
...
__name(getGoogleAdsAuthUrl, "getGoogleAdsAuthUrl");
async function exchangeGoogleAdsCode(db, code, state, env) {
  ...
  ).bind(id, organizationId, "Google Ads", GOOGLE_ADS_SLUG, "connected", config, now, now, now).run();
  return organizationId;
}
__name(exchangeGoogleAdsCode, "exchangeGoogleAdsCode");
async function listIntegrations(db, organizationId) {
...
async function disconnectIntegration(db, integrationId, organizationId) {
  const r = await db.prepare(
    "UPDATE Integration SET status = 'disconnected', config = NULL, updatedAt = ? WHERE id = ? AND organizationId = ?"
  ).bind((/* @__PURE__ */ new Date()).toISOString(), integrationId, organizationId).run();
  return (r.meta.changes ?? 0) > 0;
}
__name(disconnectIntegration, "disconnectIntegration");

// src/index.ts
var app = new Hono2();
var frontendOrigin = /* @__PURE__ */ __name((c) => c.env.FRONTEND_URL ?? "http://localhost:5173", "frontendOrigin");
app.use(
  "*",
  cors({
...
app.post("/api/auth/register", async (c) => {
  ...
  if (!name || !email || !password) {
    return c.json({ message: "Nome, e-mail e senha s\xE3o obrigat\xF3rios" }, 400);
  }
  if (password.length < 6)
    return c.json({ message: "Senha deve ter pelo menos 6 caracteres" }, 400);
  if (name.length < 2)
    return c.json({ message: "Nome deve ter pelo menos 2 caracteres" }, 400);
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email))
    return c.json({ message: "E-mail inv\xE1lido" }, 400);
  try {
    const result = await register(c.env.DB, {
...
app.post("/api/auth/login", async (c) => {
  ...
  if (!email || !password)
    return c.json({ message: "E-mail e senha s\xE3o obrigat\xF3rios" }, 400);
  try {
...
app.post("/api/auth/forgot-password", async (c) => {
  ...
  return c.json({
    message: "Se existir uma conta com esse e-mail, voc\xEA receber\xE1 as instru\xE7\xF5es."
  });
});
app.post("/api/auth/refresh", async (c) => {
  ...
  if (!refreshToken)
    return c.json({ message: "Refresh token obrigat\xF3rio" }, 400);
  try {
    const result = await refreshAccessToken(c.env.DB, {
      JWT_SECRET: c.env.JWT_SECRET,
      JWT_REFRESH_SECRET: c.env.JWT_REFRESH_SECRET
    }, refreshToken);
    return c.json(result);
  } catch {
    return c.json({ message: "Refresh token inv\xE1lido ou expirado" }, 401);
  }
});
...
app.get("/api/integrations", async (c) => {
  ...
  if (!authHeader?.startsWith("Bearer "))
    return c.json({ message: "Token n\xE3o informado" }, 401);
  const payload = await verifyAccessToken(c.env.JWT_SECRET, authHeader.slice(7));
  if (!payload)
    return c.json({ message: "Token inv\xE1lido ou expirado" }, 401);
  try {
    const list = await listIntegrations(c.env.DB, payload.organizationId);
    return c.json({ integrations: list });
  } catch {
    return c.json({ message: "Erro ao listar integra\xE7\xF5es" }, 500);
  }
});
app.get("/api/integrations/google-ads/auth-url", async (c) => {
  ...
  if (!c.env.GOOGLE_CLIENT_ID || !c.env.GOOGLE_CLIENT_SECRET) {
    return c.json({
      message: "Google Ads n\xE3o configurado. Defina GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET no Worker."
    }, 503);
  }
  ...
  try {
    ...
    return c.json({ message: "Erro ao gerar URL de autoriza\xE7\xE3o" }, 500);
  }
});
app.get("/api/integrations/google-ads/callback", async (c) => {
  ...
  if (!code || !state)
    return c.redirect(`${redirectBase}?error=missing_code_or_state`);
  const apiBaseUrl = c.env.API_BASE_URL ?? new URL(c.req.url).origin;
  try {
    const organizationId = await exchangeGoogleAdsCode(
      c.env.DB,
      code,
      state,
      {
...
app.delete("/api/integrations/:id", async (c) => {
  ...
  if (!id)
    return c.json({ message: "ID da integra\xE7\xE3o obrigat\xF3rio" }, 400);
  try {
    const ok = await disconnectIntegration(c.env.DB, id, payload.organizationId);
    if (!ok)
      return c.json({ message: "Integra\xE7\xE3o n\xE3o encontrada" }, 404);
    return c.json({ success: true });
  } catch {
    return c.json({ message: "Erro ao desvincular" }, 500);
  }
});
app.all("*", (c) => c.json({ message: "Rota n\xE3o encontrada" }, 404));
var src_default = app;
...
// .wrangler/tmp/bundle-jdwfoc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jdwfoc/middleware-loader.entry.ts
