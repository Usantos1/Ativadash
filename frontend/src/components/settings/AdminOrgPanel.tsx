...
/**
 * Conteúdo técnico da antiga rota `/admin`, reutilizável em `/configuracoes/admin`.
 */
export function AdminOrgPanel() {
  const user = useAuthStore((s) => s.user);
  const memberships = useAuthStore((s) => s.memberships);
  const org = user?.organization;
  const mode = resolveAppNavMode(user ?? null);
  const hideTeamLink =
    shouldEnforceAgencyBranchRouteGuard(user, memberships) &&
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground">
          ...
          {hideTeamLink ? (
            <p className="text-xs">Gestão de equipe neste perfil é feita pela matriz ou em outro contexto.</p>
          ) : (
        ...
        <CardContent>
          ...
          !isAgencyBranchExpandedOpsEnabled() ? (
            <p className="text-sm text-muted-foreground">
              Integrações desta filial: use o menu <strong className="text-foreground">Conexões</strong>.
            </p>
          ) : (
            <Link
