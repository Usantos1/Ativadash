...
import { Activity, Building2, Link2, Mail, RefreshCw } from "lucide-react";
import type { GoogleAdsSetupDto } from "@/lib/integrations-api";
import type { IntegrationFromApi } from "@/lib/integrations-api";
...
type Props = {
  ...
  setup: GoogleAdsSetupDto | null;
};

export function GoogleAdsSummaryPanel({ connected, row, setup }: Props) {
  if (!connected) {
    return (
      <aside className="rounded-2xl border border-dashed border-border/70 bg-muted/20 p-6 text-center shadow-inner">
        <p className="text-sm font-medium text-muted-foreground">Conecte o Google Ads para ver o resumo da integração.</p>
      </aside>
    );
  }
    ...
    <aside className="space-y-1 rounded-2xl border border-border/60 bg-card p-5 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.12)] ring-1 ring-black/[0.02] dark:ring-white/[0.04] sm:p-6">
      ...
      <div className="divide-y divide-border/50">
        ...
        <SummaryRow
          ...
          label="Contas acessíveis"
          value={String(setup?.accessibleCount ?? 0)}
        />
        <SummaryRow
          icon={<Link2 className="h-4 w-4" />}
          label="Vínculos com clientes"
          value={String(setup?.assignmentCount ?? 0)}
        />
        <SummaryRow
          icon={<Building2 className="h-4 w-4" />}
          label="Padrão da organização"
          value={setup?.defaultCustomerId ? fmtCid(setup.defaultCustomerId) : "Não definido"}
          mono
        />
      </div>
    </aside>
