...
import type { GoogleAdsAdRow } from "@/lib/integrations-api";
import type { BusinessGoalMode } from "@/lib/business-goal-mode";

export function GoogleAdsLevelTable({
  rows,
  businessGoalMode,
  loading,
  emptyLabel,
  ...
  return (
    <DataTablePremium zebra stickyHeader minHeight="min-h-[200px]" className="text-[13px]">
      <thead>
        <tr>
          <th className="text-left">ID anúncio</th>
      ...
      <tbody>
        {rows.slice(0, 150).map((row, i) => {
          ...
          return (
            <tr key={`${row.adId}-${row.adGroupName}-${i}`}>
              <td className="max-w-[100px] truncate font-mono text-xs text-foreground">
                {row.adId || "—"}
              </td>
              <td className="max-w-[160px] truncate text-muted-foreground">{row.adGroupName}</td>
              <td className="max-w-[160px] truncate text-muted-foreground">{row.campaignName}</td>
